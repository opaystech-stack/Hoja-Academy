...
export default function ListRow8({ d, meta, styles }: { d: ListRow8Data; meta: DittoNodeMetaMap; styles: ListRow8Styles }) {
  return (
    <li data-ditto-id={meta[0]?.anchor} className="flex relative min-w-0">
      <a data-ditto-id={meta[1]?.anchor} className={cn("flex relative min-w-0 py-3 items-center grow text-background text-[0.875rem] leading-px tracking-[0.1px] whitespace-nowrap text-nowrap cursor-pointer max-md:text-[0.75rem]", styles.className)} href={d.href}>
        {d.label}
      </a>
    </li>
  );
