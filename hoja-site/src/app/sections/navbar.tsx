    ...
    <header className="block before:content-[''] before:table before:w-0 before:h-0" data-ditto-id="style-header">
      <header className={`h-28 min-h-28 flex fixed inset-x-0 z-9999 max-w-full px-[1.6rem] justify-around items-center gap-x-5 max-md:h-17.5 max-md:min-h-17.5 max-md:pt-[1.175rem] max-md:pr-[11.3px] max-md:pl-[1.175rem] max-md:flex-wrap max-lg:justify-center max-lg:items-start max-md:self-start max-md:gap-x-[initial] md:max-lg:pt-[30.7px] md:max-lg:px-[0.9625rem] 2xl:px-[2.4rem] 2xl:justify-between 2xl:gap-x-[initial] transition-colors duration-300 ${scrolled ? "bg-[rgba(9,19,26,0.96)] backdrop-blur-[6px]" : ""}`}>
        ...
        <div className="w-[23.5%] flex relative min-w-0 flex-col justify-center items-start max-md:w-[52%] max-md:z-999 max-md:flex-wrap md:max-lg:w-[47%] 2xl:w-[21%]">
          <a className="inline-block text-color-001 [font-family:Montserrat,_sans-serif] text-[0.9375rem] font-medium leading-[1.1875rem] coursr-pointer" href="/" aria-label="Hoja Academy — accueil">
            ...
            <ul className="flex relative z-2 items-center justify-center leading-6 [list-style-type:none] list-outside">
              <li ref={liRef} className="h-12.5 flex relative" onMouseEnter={hoverIn} onMouseLeave={hoverOut}>
                <button
                  ...
                  className="h-12.5 flex relative mx-[0.5px] px-4 items-center text-color-001 [font-family:Montserrat,_sans-serif] text-sm leading-12.5 whitespace-nowrap coursr-pointer hover:text-primary focus-visible:text-primary outline-none"
                >
                  Formations
                    ...
                    {FORMATIONS.map((f) => (
                      <li key={f.href} className="block relative">
                        <a
                          ...
                          className={`flex relative py-[0.8125rem] px-5 items-center text-color-001 [font-family:Montserrat,_sans-serif] text-[0.9375rem] font-medium leading-5 whitespace-nowrap coursr-pointer hover:text-primary focus-visible:text-primary ${isF(f.href) ? "font-bold" : ""}`}
                        >
              ...
              {NAV.map((n) => (
                <li key={n.href} className="h-12.5 flex relative">
                  <a
                    ...
                    className={`h-12.5 flex relative mx-[0.5px] px-4 items-center text-color-001 [font-family:Montserrat,_sans-serif] text-sm leading-12.5 whitespace-nowrap coursr-pointer hover:text-primary focus-visible:text-primary ${path === n.href ? "font-bold underline decoration-primary underline-offset-8" : ""}`}
                  >
                    {n.label}
                  </a>
                </li>
        ...
        <div className="w-[19.925rem] flex relative min-w-0 flex-col items-end gap-5 max-md:hidden md:max-lg:w-[17.8rem] 2xl:w-[313.3px]">
          <a className="inline-block py-[0.9375rem] px-7.5 rounded-[60px] text-color-001 [font-family:Montserrat,_sans-serif] text-[0.9375rem] font-semibold leading-[0.9375rem] tracking-[-0.2px] text-center capitalize bg-primary coursr-pointer hover:bg-[#1c8f6a] md:max-lg:text-sm 2xl:text-lg" href="/postuler">
            Postuler
          </a>
        </div>
        ...
        <div className="hidden max-lg:block relative z-999" >
          <button
            ...
            className="flex relative min-w-0 min-h-[44px] min-w-[44px] p-[6.5px] rounded-[8px] justify-center items-center text-background bg-[rgba(255,255,255,0.10)] coursr-pointer"
          >
            ...
            <ul className="block [list-style-type:none]">
              <li className="block border-b border-solid border-surface-4">
                <button
                  ...
                  className="w-full flex items-center justify-between py-3.5 text-color-001 [font-family:Montserrat,_sans-serif] text-base font-medium coursr-pointer"
                >
                  Formations
                ...
                {mobileF && (
                  <ul className="pb-2 [list-style-type:none]">
                    {FORMATIONS.map((f) => (
                      <li key={f.href}>
                        <a href={f.href} aria-current={isF(f.href) ? "page" : undefined} className={`block py-3 pl-4 text-color-001 text-[0.9375rem] ${isF(f.href) ? "font-bold" : ""}`}>
                          {f.label}
                        </a>
                      </li>
                    ))}
              ...
              {NAV.map((n) => (
                <li key={n.href} className="block border-b border-solid border-surface-4">
                  <a href={n.href} className="block py-3.5 text-color-001 [font-family:Montserrat,_sans-serif] text-base font-medium">
                    {n.label}
                  </a>
                </li>
              ))}
